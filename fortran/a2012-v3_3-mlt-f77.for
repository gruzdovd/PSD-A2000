*  A2012 - ver.3.3-mlt
*  15/08/2023 - IGRF13 coefficients in DIPGARM has been implemented
*  19/08/2023  - SM coordinates, and MLT in the field line are calculated

      subroutine a2000f(ut,iy,mo,id,ro,v,bimf,dst,al,x0,bm,bdd)
*-----------------------------------------------------------------
...
*                           (February 2012)
*
* The dynamic paraboloid model (A2000_1) allows to calculate the
...
*     by Shue et al. 1997 and Shue et al. 1998
*
	  Psw=(1.67e-6)*ro*V*V              ! Solar wind pressure [nPa]          
      bz=bimf(3)
c      if (bz.ge.0.) then
c      r1=(11.4+0.013*bz)*(psw**(-1./6.6))
c      else
c      r1=(11.4+0.14*bz)*(psw**(-1./6.6))
c      end if                                                 ! Shue 1997
      r1=(10.22+1.29*tanh(0.184*(8.14+bz)))*(psw**(-1./6.6))  ! Shue 1998      
*
***>  calculation of the distance to the geotail inner edge: R2 [Re]
*
      if(dst.ge.-10.0.or.r1.ge.12. )then
      R2=0.7*r1
      goto 3
      end if
...
*       COMMON BLOCK /ddd/).                                  .
***************************************************************
      COMMON/TRAN/g2gsm(3,3),UTt,IDAYt,iyeart,datt
      common /ddd/ sind,cosd
      dimension gauss(3) 
      utt=ut
      idayt=iday
      iyeart=iyear	  
      datt=iyeart*1.+idayt/365.
      Ihour=int(ut)
      dmin=(ut-ihour)*60.
      min=int(dmin)
...
*   year>=1900  							 *
*   Last valid IGRF values are for 2025.					 *
*   Written by V. Kalegaev						 *
*      Last updated 16.08.2023
*------------------------------------------------------------------------*
      dimension gauss(3), g(75), sg10(3)
      data g/
     *-31543., -2298.,  5922.,
     *-31464., -2298.,  5909.,
     *-31354., -2297.,  5898.,
     ...
     *-29775., -1848.,  5406.,
     *-29692., -1784.,  5306.,
     *-29619.4, -1728.2, 5186.1,
     *-29554.63, -1669.05, 5077.99,
     *-29496.57,-1586.42, 4944.26,
     *-29441.46,-1501.77, 4795.99,
     *-29404.8, -1450.9,  4652.5/
      data sg10/5.7,     7.4,   -25.9/
      i1=(iyear-1900)
      i2=i1/5
      i3=i1-5*i2
      n=(i2)*3+1
...
* x0sm - solar-magnetic coordinates
      ...
      x0sm(3)=sign(1.,lat)*sqrt(1-x0sm(1)*x0sm(1)-x0sm(2)*x0sm(2))
c	  print *, lat, x0sm
        call smtogsm(SM2GSM)
...
****************************************************************
     ...
     *BD,bd0
      ...
      SM2GSM(3,2)=0.
      SM2GSM(3,3)=CPSI
c	  PRINT *, 'SMtoGSM: CPSI=', CPSI, 'SPSI=', SPSI
      return
      END
C
...
*****************************************************************
      ...
      SSD =x1 ! IGRF/dipole field on/off (2/1/0)
      ...
      SS2 = 0.! x7 - Region 2 FAC field - turned "OFF"
      SIMF=x7 ! IMF on/off (1/0)
      SSCP= 1. ! bounce motion of guided center on/off (1/0)- turned "ON"
      SSP = 0.! x10 - drift motion of guided center on/off (1/0) - turned "OFF"
...
12    EZ(1,1)=0.                                                        
      ...
      UZ(I,J)=ZU(J,I)                                                   
7     CONTINUE                                                          
6     CALL PERE(V1,B1,ZU) ! B1=Bd+Bcf when al>al1; B1=Bcf when al<al1
        call bdipc(uf,b0,b1d) ! b1d - dipole "corrected" by RC at R>6.6
        
c     calculation of Bcf = Bcfd+Bcfr		
		if (al.ge.al1) then
        do i=1,3
        b1cf(i)=b1(i)-b1d(i)
        end do
...
c     calculation of BR
	call bring(ff)
      ...
      If (ssd.eq.2.) then
	  call pere2(uf,xg,g2gsm,-1)
      call igrf13(datt,xg,Bigrf,amodB)
      call pere2(Bigrf,Bigr,g2gsm,1)
      end if
       
        do i=1,3
        if (ssd.ne.2.) then
        b1d(i)=ssd*b1d(i)*bd0                 ! B dipole calculation
        else
        b1d(i)=Bigr(i)                        ! B IGRF calculation
        end if
c     calculation of BR, Bcf
        b1cfd(i)=b1cf(i)*bd0
      ...
      CALL PERE(B2A,B2,A2X)     ! Tail current mf                                        
...
11    b(j)=b1d(j)+b1cf(j)+b1rc(j)+b2(j)+B3(j)+bfac1(j)+bfac2(j)
...
************************************************************************
 ...
 30   CONTINUE
      ...
      if (iprint.eq.1) write (*,123) Y,AB,B
...
************************************************************************
     ...
     *BD,bd0
      ...
      COMMON/BEGFC/B1CF(3),B1CFD(3),B1CFR(3),B1D(3),B1RC(3)
      DIMENSION X0(3),FF(3), xsm(3), SM2GSM(3,3)

c      open (1, file='output1.dat')
 ...
 1    CONTINUE
      ...
      if (iprint.eq.1) then
c	  write (*,*)
c      write (*,123) Y,AB,B,rmlt, R
          num_points = num_points + 1
          output_array(1, num_points) = Y(1)
          output_array(2, num_points) = Y(2)
          output_array(3, num_points) = Y(3)
          output_array(4, num_points) = AB
          output_array(5, num_points) = B(1)
          output_array(6, num_points) = B(2)
          output_array(7, num_points) = B(3)
          output_array(8, num_points) = rmlt
          output_array(9, num_points) = R		  
      end if
 ...
 2    CONTINUE
      call pere2 (y,xsm,sm2gsm,-1)
      rfi=Atan(xsm(2)/xsm(1))*180./PI 
      IF (xsm(1).LT.0.) rfi=180.+rfi
      IF (xsm(2).LT.0..and.xsm(1).gt.0.) rfi=360.+rfi
      rmlt=12.+rfi/15.
...
c      if((y(3)+z0)*(y(3)+z0)+y(2)*y(2).le.0.01) then
c      y(2)=y(2)+0.2
c      call FIELD(Y,FF)	  
...
*-----------------------------------------------------------------------
      IF (ssp.eq.1.) goto 8          !guiding center trajectory
      IF (al.le.al0) goto 8          ! geomagnetic equator
      IF (abs(y(3)+z0).gt.abs(h)/2) goto 8
      IF (zn*h*(y(3)+z0)*b(3).gt.0.) goto 8
cc      if (iprint.eq.1) write (*,123) Y,AB,B
        di=1.
        IF (y(2).eq.0.) THEN
        call tail_tr(z0,ttcs,di)
cc        if (iprint.eq.1) write (*,123) ttcs, ab
c        write (*,*) ttcs, '----current sheet'
        call field(y,ff)
        goto 8
        end if
      hz=abs(b(3)*h/ab)
...
cc        if (iprint.eq.1) write (*,123) teq, ab
        write (5,*) teq, '----GSM equator'
        call field(y,ff)
*-----------------------------------------------------------------------
...
c       write (*,123) Y,AB,B, rmlt, R
          ...
          output_array(8, num_points) = rmlt
          output_array(9, num_points) = R
c          print *, "Точка:", num_points, "Y=", Y(1), Y(2), Y(3)
c          print *, "X:", output_array(1, num_points)
c          print *, "Y:", output_array(2, num_points)
c          print *, "Z:", output_array(3, num_points)
c          print *, "AB:", output_array(4, num_points)
c          print *, "B1:", output_array(5, num_points)
c          print *, "B2:", output_array(6, num_points)
c          print *, "B3:", output_array(7, num_points)
c          print *, "MLT:", output_array(8, num_points)
c          print *, "R:", output_array(9, num_points)
        end if		  
        call field(y,ff)
      TIN=TIN+P*(CP+SP)
      SP=CP
...
124   FORMAT(/,(5X,1HX,7X,1HY,7X,1HZ,3X,1H|,8X,2HAB,
     *3x,1H|,9X,2HBX,10X,2HBY,10X,2HBZ))
123   FORMAT(3F8.3,1X,1H|,F12.3,1x,1H|,2x,3F12.3,4X, 2F6.2)

      return
      END
C
      SUBROUTINE mas1d (flux,Br,Rs,Rt)
**********************************************************************
...
C*************************************************************
     ...
     *BD,bd0
      ...
      IF (R.GT.1.AND.R.LT.2.*R1) GOTO 3
       P(1)= 0.
       P(2)= 0.
       P(3)= 0.
                              RETURN
